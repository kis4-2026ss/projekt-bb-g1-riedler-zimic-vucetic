...
describe('Database Setup & Models', () => {
  ...
  afterEach(() => {
    // Clean up temporary database files if created
    if (fs.existsSync(tempDbPath)) {
      try {
        fs.unlinkSync(tempDbPath);
      } catch (err) {
        // Ignore error if locked or busy
      }
    }
    const dbDir = path.dirname(tempDbPath);
    if (fs.existsSync(dbDir)) {
      try {
        // Do not delete db directory as other files might be there, 
        // but try to clean up if it's empty
  ...
  describe('initializeDatabase', () => {
    ...
    it('should not seed the database if it is not empty', async () => {
      ...
      // Run initializeDatabase with seed: true
      await initializeDatabase({
        sequelize,
        models,
      ...
      // It should only have the existing wishlist, not the seed one
      const wishlists = await models.Wishlist.findAll();
      expect(wishlists.length).toBe(1);
...
describe('API Endpoints', () => {
  ...
  beforeEach(async () => {
    // Start with a clean in-memory database for each test to ensure isolation
